...
//  Format: [ { "room": "rool", "alias": [ "alias1", "alias2", ... ] }, ... ]
...
function setPresets() {
    ...
    if (options.useHomePresets &&
        ...
        myPresetAliases = home.presets || [];
    }
    else {
        console.log('Using env.PRESETS');
        try {
            myPresetAliases = options.presetAliases ? parseJSONArray(options.presetAliases, "PRESET_ALIASES") : [];
            myPresets = options.presets ? parseJSONArray(options.presets, "PRESETS") : [];
        }
        catch (err) {
            console.log('ERR', err);
        }
    }
    if (myPresetAliases.length > 0) {
        for (var i = 0; i < myPresetAliases.length; i++) {
            myPresets.push(myPresetAliases[i].name);
            myPresets = myPresets.concat(myPresetAliases[i].alias);
        }
    }
    if (options.debug) {
        console.log("myPresets: " + myPresets);
        console.log("myPresetAliases: " + JSON.stringify(myPresetAliases));
    }
}

function findAlias(name, aliases) {
    var res = name;
    try {
    ...
    if (options.debug) {
        console.log("FIND ALIAS: " + name + " -> " + res);
    }
    return res;
}

function findRoomAlias(room) {
    if (options.debug) {
        console.log("FIND ROOM ALIAS: " + room);
    }
    return findAlias(room, myRoomAliases);
}

function findPresetAlias(preset) {
    if (options.debug) {
        console.log("FIND PRESET ALIAS: " + preset);
    }
    return findAlias(preset, myPresetAliases);
}
...
EchoSonos.prototype.intentHandlers = {
    ...
    PlayPresetIntent: function (intent, session, response) {
        ...
        var preset = slotValue(intent.slots.Preset, true).toLowerCase();
        var room = checkRoomSlot(intent);
        options.path = '/preset/' + encodeURIComponent(slotValue(intent.slots.Preset, true).toLowerCase());
        if (room) {
    ...
    PlaylistIntent: function (intent, session, response) {
        ...
        loadCurrentRoomAndService('DefaultEcho', room, function (room, service) {
            playlistHandler(room, slotValue(intent.slots.Preset, true), 'playlist', response);
        });
    },
    ...
    FavoriteIntent: function (intent, session, response) {
        ...
        loadCurrentRoomAndService('DefaultEcho', room, function (room, service) {
            playlistHandler(room, slotValue(intent.slots.Preset, true), 'favorite', response);
        });
    },
    ...
    ChangeRoomAndServiceIntent: function (intent, session, response) {
        ...
        if (!options.advancedMode) {
            ...
            changeCurrent('DefaultEcho', checkRoomSlot(intent), slotValue(intent.slots.Service), function () {
                genericResponse('', response);
            });
        }
    },
        ...
        loadCurrentRoomAndService('DefaultEcho', room, function (room, service) {
            ...
            httpreq(options, function (error, responseJson) {
                if (!error) {
                    ...
                    var responseText = STATE_RESPONSES[randResponse].replace("$currentTitle",
                                                                             responseJson.currentTrack.title).replace("$currentArtist",
                                                                                                                      responseJson.currentTrack.artist);
...
function isPreset(p) {
    var res = myPresets.indexOf(p) != -1;
    if (options.debug) console.log('isPreset(' + p + ') -> ' + res);
    return res;
}
...
function musicHandler(roomValue, service, cmdpath, name, response) {
    ...
    if (isPreset(p) || (service == 'presets')) {
        if (isPreset(p)) {
            options.path = '/preset/' + encodeURIComponent(findPresetAlias(p));
            httpreq(options, function (error) {
                genericResponse(error, response);
...
function parseRoomAndGroup(roomArgument) {
    ...
    if (groupIndex && (groupIndex + 4 == (roomArgument.length - 1)) && roomArgument.length >= 7) {
        ...
        roomArgument = roomArgument.substr(0, groupIndex - 1);
    }
    roomAndGroupParsed.room = roomArgument ? findRoomAlias(roomArgument) : false; // >>> KSH
    return roomAndGroupParsed;
}
